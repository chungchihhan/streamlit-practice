...
for i, doc in enumerate(docs):
    ...
    updated_content = col1.text_area(
        f"Original Page {doc.metadata['page']} Content:",
...
updated_file_name = (
    col2.text_input("Enter the updated file name:", value=f"Updated_{options[:-4]}")
    + ".txt"
)
...
for i, doc in enumerate(updated_docs_content):
    col2.write(f"Page {i}")
    col2.text_area(
        f"Edited Page {i} content :",
        value=updated_docs_content[doc],
        height=200,
        key=f"{i}+updated",
        disabled=True,
    )
